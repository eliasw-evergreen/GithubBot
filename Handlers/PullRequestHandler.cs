    ...
    private async Task HandleOpenActions(string action, PullRequest pr, Repository repo, CancellationToken ct)
    {
            ...
            if (stored.MessageId != 0)
            {
                ...
                if (originalMsg != null)
                {
                    await _discord.EditMessageAsync(channel.Id, stored.MessageId,
                        $"{rolePrefix}{mention} opened a PR in **[{repo.Name}]({repo.HtmlUrl})**", embed);
        ...
        if (msg != null)
        {
            ...
            var threadId = await _discord.CreateThreadAsync(channel.Id, msg.Id,
                $"PR #{pr.Number} — {pr.Title}", ct);
            _prMap.Set(pr.NodeId, new PrMapEntry { MessageId = msg.Id, ThreadId = threadId, PrNumber = pr.Number, PrTitle = pr.Title, AuthorLogin = pr.User?.Login, IsDraft = pr.Draft == true, RepoFullName = repo.FullName });
        }
    }
